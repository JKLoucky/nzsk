...
NZ_AGENT_SERVICE="/etc/systemd/system/nezha-agent1.service"
NZ_AGENT_SERVICERC="/etc/init.d/nezha-agent1"
...
install_amzayo_theme() {
    ...
    if [[ $IS_DOCKER_NEZHA == 1 ]]; then
        ...
        cp -f resource/template/theme-custom/* ${NZ_DASHBOARD_PATH}/resource/template/theme-custom/
        cp -f resource/static/custom/* ${NZ_DASHBOARD_PATH}/resource/static/custom/
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ]; then
        ...
        systemctl enable nezha-agent1
        systemctl restart nezha-agent1
    else
...
show_agent_log() {
    ...
    if [ "$os_alpine" != 1 ]; then
        journalctl -xf -u nezha-agent1.service
    else
        echo "很抱歉，OpenRC 无此功能"
    fi
...
uninstall_agent() {
    ...
    if [ "$os_alpine" != 1 ]; then
        systemctl disable nezha-agent1.service
        systemctl stop nezha-agent1.service
...
restart_agent() {
    ...
    if [ "$os_alpine" != 1 ]; then
        systemctl restart nezha-agent1.service
    else
        rc-service nezha-agent restart
    fi
